...
if FF == 'V':
    ...
    mom,pref,mom2,pref2=GetCombs.get_moms_and_prefacs_A2(md**2)
...
# Calculate Ratio
if FF == 'A2':
    avn0,errn0 =Ratio.build_Ratio_A2(jb3pt,jb3pt2,jbdx,jbdy,jbdz,jbb,pref,pref2,dt,nsq,nconf,ed,mb,pre,dsfit,bsfit)
else:
    avn0,errn0 =Ratio.build_Ratio(jb3pt,jbdx,jbdy,jbdz,jbb,pref,dt,nsq,nconf,ed,mb,pre,dsfit,bsfit)
...
for t1 in range(int(ts/2-1-reg_low-cut)):
    for t2 in range(int(ts/2-1-reg_low-cut)):
        ...
        for i in range(nconf):  
            x=x+(((Basic.sum_with_prefacs_jack(av1n0,nsq, t1+reg_low, i))/(np.sqrt(1/3*(Basic.jack(avdx[t1+reg_low],i)+Basic.jack(avdy[t1+reg_low],i)+Basic.jack(avdz[t1+reg_low],i))*Basic.jack(avb[dt-(t1+reg_low)],i))))*np.sqrt((4*dsfit['EffectiveMass'][i]*bsfit['EffectiveMass'][i])/(np.exp(-dsfit['EffectiveMass'][i]*(t1+reg_low))*np.exp(-bsfit['EffectiveMass'][i]*(dt-(t1+reg_low)))))*pre-avn0[t1])*(((Basic.sum_with_prefacs_jack(av1n0,nsq, t2+reg_low, i))/(np.sqrt(1/3*(Basic.jack(avdx[t2+reg_low],i)+Basic.jack(avdy[t2+reg_low],i)+Basic.jack(avdz[t2+reg_low],i))*Basic.jack(avb[dt-(t2+reg_low)],i))))*np.sqrt((4*dsfit['EffectiveMass'][i]*bsfit['EffectiveMass'][i])/(np.exp(-dsfit['EffectiveMass'][i]*(t2+reg_low))*np.exp(-bsfit['EffectiveMass'][i]*(dt-(t2+reg_low)))))*pre-avn0[t2])            
        covmat[t1][t2]=x
        covmat[t2][t1]=x  
...
def jackmass(t1,i):
    return (((Basic.sum_with_prefacs_jack(av1n0,nsq, t1, i)))/(np.sqrt(1/3*(Basic.jack(avdx[t1],i)+Basic.jack(avdy[t1],i)+Basic.jack(avdz[t1],i))*Basic.jack(avb[dt-t1],i))))*np.sqrt((4*md*mb)/(np.exp(-md*(t1))*np.exp(-mb*(dt-(t1)))))*pre
