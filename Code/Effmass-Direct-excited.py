...
def chi(a1, m0, a2, dm):
    m1 = m0 + abs(dm)  # enforce m1 > m0
    model = [
        a1 * (np.exp(-m0 * t) + np.exp(-m0 * (ti - t))) +
        a2 * (np.exp(-m1 * t) + np.exp(-m1 * (ti - t)))
        for t in lst
    ]
...
best_dm = mbar.values["dm"]
best_m1 = best_m0 + best_dm


print('Initial guess: A0 = {}, m0 = {}'.format(a0tmp, masstmp))
print('Initial guess: A1 = {}, m2 = {}'.format(np.sqrt(a0tmp), masstmp*1.5))
print("Optimized a0:", mbar.values["a1"])
print("Optimized a1:", mbar.values["a2"])
print("Optimized m0:", mbar.values["m0"])
print("Optimized m1:", mbar.values["dm"] + mbar.values["m0"])
...
full_fit = [
    best_a1 * (np.exp(-best_m0 * t) + np.exp(-best_m0 * (ti - t))) +
    best_a2 * (np.exp(-best_m1 * t) + np.exp(-best_m1 * (ti - t)))
    for t in full_range
]
