...
nsq0=pd.read_csv('./Mass-Ds0.259-0.csv', sep=r'\s+', engine='python')
nsq1=pd.read_csv('./Mass-Ds0.259-1.csv', sep=r'\s+', engine='python')
nsq2=pd.read_csv('./Mass-Ds0.259-2.csv', sep=r'\s+', engine='python')
nsq3=pd.read_csv('./Mass-Ds0.259-3.csv', sep=r'\s+', engine='python')
nsq4=pd.read_csv('./Mass-Ds0.259-4.csv', sep=r'\s+', engine='python')
nsq5=pd.read_csv('./Mass-Ds0.259-5.csv', sep=r'\s+', engine='python')

nsq0plt=pd.read_csv('./Ds0.259Result-0-uf.csv', sep=r'\s+', engine='python')
nsq1plt=pd.read_csv('./Ds0.259Result-1-uf.csv', sep=r'\s+', engine='python')
nsq2plt=pd.read_csv('./Ds0.259Result-2-uf.csv', sep=r'\s+', engine='python')
nsq3plt=pd.read_csv('./Ds0.259Result-3-uf.csv', sep=r'\s+', engine='python')
nsq3plt=pd.read_csv('./Ds0.259Result-3-uf.csv', sep=r'\s+', engine='python')
nsq4plt=pd.read_csv('./Ds0.259Result-4-uf.csv', sep=r'\s+', engine='python')
nsq5plt=pd.read_csv('./Ds0.259Result-5-uf.csv', sep=r'\s+', engine='python')
...
low0  = int(reg_low0.iloc[0])
high0 = int(reg_up0.iloc[0]) + 1


plt.plot(x0,y0,color='g',linewidth=0.5)
plt.fill_between(list(range(47))[low0:high0], nsq0plt['EffectiveMass']+sigma0, nsq0plt['EffectiveMass']-sigma0, color='g',alpha=0.2)
plt.annotate(r'$\bf{preliminary}$',xy=(0.65,0.03),xycoords='axes fraction',fontsize=15,color='grey',alpha=1)

low1  = int(reg_low1.iloc[0])
high1 = int(reg_up1.iloc[0]) + 1

plt.plot(x1,y1, color='b',linewidth=0.5)
plt.fill_between(list(range(47))[low1:high1], nsq1plt['EffectiveMass']+sigma1, nsq1plt['EffectiveMass']-sigma1, color='b',alpha=0.2)
plt.plot(x2,y2,color='orange',linewidth=0.5)

low2  = int(reg_low2.iloc[0])
high2 = int(reg_up2.iloc[0]) + 1

plt.fill_between(list(range(47))[low2:high2], nsq2plt['EffectiveMass']+sigma2, nsq2plt['EffectiveMass']-sigma2, color='orange',alpha=0.2)
plt.ylim(bottom=0.71)
plt.ylim(top=0.88)

low3  = int(reg_low3.iloc[0])
high3 = int(reg_up3.iloc[0]) + 1

plt.plot(x3,y3, color='brown',linewidth=0.5)
plt.fill_between(list(range(47))[low3:high3], nsq3plt['EffectiveMass']+sigma3, nsq3plt['EffectiveMass']-sigma3, color='brown',alpha=0.2)

low4  = int(reg_low4.iloc[0])
high4 = int(reg_up4.iloc[0]) + 1

plt.plot(x4,y4, color='red',linewidth=0.5)
plt.fill_between(list(range(47))[low4:high4], nsq4plt['EffectiveMass']+sigma4, nsq4plt['EffectiveMass']-sigma4, color='red',alpha=0.2)

low5  = int(reg_low5.iloc[0])
high5 = int(reg_up5.iloc[0]) + 1

plt.plot(x5,y5, color='cyan',linewidth=0.5)
plt.fill_between(list(range(47))[low5:high5], nsq5plt['EffectiveMass']+sigma5, nsq5plt['EffectiveMass']-sigma5, color='cyan',alpha=0.2)
...
plt.axvline(x=32, color='grey', linestyle='--', linewidth=0.8)  # At x=4
