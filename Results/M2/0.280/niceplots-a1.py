...
# Originaldaten + Fits
for i in nsq:
    plt.errorbar(range(25), nsq[i][0][:25], yerr=nsq[i][1][:25],
...
for i in nsq_disp:
    plt.errorbar(range(25), nsq_disp[i][0][:25], yerr=nsq_disp[i][1][:25],
...
plt.axis((0,25,0.15,0.45))
plt.tick_params(axis='both', which='major', labelsize=14)
plt.legend(fontsize=10, ncol=2, markerscale=0.8)
plt.savefig('Niceplot-A1-Disp.pdf', transparent=True, dpi=250, bbox_inches='tight')
...
plt.errorbar(list(range(25)), nsq0[0][0:25], yerr=nsq0[1][0:25],ls='none',fmt='x',label='$n^2=0$',color='g')
...
#plt.yscale('log')
plt.legend()
plt.savefig('Niceplot-A1.pdf',transparent=True,dpi=250,bbox_inches='tight')
'''