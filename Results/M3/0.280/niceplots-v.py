...
md_vals = {
    1: 0.7458868408253149,
    2: 0.7567413325078149,
    3: 0.767,  # dummy
    4: 0.77745605,
    5: 0.787656860351565
}
...
# Normale Daten
for i in nsq:
    plt.errorbar(range(1, 25), nsq[i][0][1:25], yerr=nsq[i][1][1:25],
    ...
    plt.plot([-1, 27], [eff, eff], color=base_colors[i], linewidth=0.5)
    plt.fill_between(range(47)[reg_low:reg_up+1], eff + sigma, eff - sigma,
                     color=base_colors[i], alpha=0.2)

# Disp-Daten
for i in nsq_disp:
    plt.errorbar(range(1, 25), nsq_disp[i][0][1:25], yerr=nsq_disp[i][1][1:25],
    ...
    plt.plot([-1, 27], [eff, eff], color=disp_colors[i], linewidth=0.5)
    plt.fill_between(range(47)[reg_low:reg_up+1], eff + sigma, eff - sigma,
...
plt.savefig('Niceplot-V-Disp.pdf', transparent=True, dpi=250, bbox_inches='tight')
...
plt.errorbar(list(range(29)), nsq1[0][1:25], yerr=nsq1[1][1:25],ls='none',fmt='x',label='$n^2=1$',color='b')
plt.errorbar(list(range(29)), nsq2[0][1:25], yerr=nsq2[1][1:25],ls='none',fmt='x',label='$n^2=2$',color='orange')
plt.errorbar(list(range(29)), nsq3[0][1:25], yerr=nsq3[1][1:25],ls='none',fmt='x',label='$n^2=3$',color='brown')
plt.errorbar(list(range(29)), nsq4[0][1:25], yerr=nsq4[1][1:25],ls='none',fmt='x',label='$n^2=4$',color='red')
plt.errorbar(list(range(29)), nsq5[0][1:25], yerr=nsq5[1][1:25],ls='none',fmt='x',label='$n^2=5$',color='magenta')
...
plt.tick_params(axis='both', which='major', labelsize=14) 
#plt.axis((0,25,0.05,0.11))

#plt.yscale('log')
plt.legend()
plt.savefig('Niceplot-V.pdf',transparent=True,dpi=250,bbox_inches='tight')
'''